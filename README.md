<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Roboto+Mono&weight=500&size=13&duration=3000&pause=1000&color=00FF88&center=true&vCenter=true&width=520&lines=Kavramsal+Mimari...;Consciousness+Curvature+Framework;Zero+Trust.+Zero+Compromise.+Full+Depth." alt="Typing SVG" />

# Fatih Yenen

[![ORCID](https://img.shields.io/badge/ORCID-0009--0000--4220--0041-a6ce39?style=flat&logo=orcid&logoColor=white)](https://orcid.org/0009-0000-4220-0041)
[![Website](https://img.shields.io/badge/sonra.art-000000?style=flat&logo=vercel&logoColor=white)](https://www.sonra.art)
[![Email](https://img.shields.io/badge/researcher%40sonra.art-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:researcher@sonra.art)
[![X](https://img.shields.io/badge/%40fatihynn-000000?style=flat&logo=x&logoColor=white)](https://x.com/fatihynn)
[![GitHub](https://img.shields.io/badge/%40fthynn-181717?style=flat&logo=github&logoColor=white)](https://github.com/fthynn)

[![Portfolio](https://img.shields.io/badge/PORTFOLIO%20SITE-21262d?style=for-the-badge&logo=windowsterminal&logoColor=c8d0e0)](https://fthynn.github.io/fthynn/)

[![Manifesto](https://img.shields.io/badge/☾★_EPİSTEMİK_MANIFESTO-E30A17?style=for-the-badge&logoColor=white)](https://fthynn.github.io/fthynn/Epistemik%20Egemenlik%20Manifestosu.html)

</div>

---

<details open>
<summary><strong>🇹🇷 Türkçe</strong></summary>

**Interdisipliner Yaratıcı Teknolog · Bilinç Araştırmacısı**

*Zamanı tüketmek için değil; kıvrımını değiştirmek için buradayım.* ☕

**SonRa ART** kurucusu · **CCFy** geliştiricisi. Bilinci, yapay zekâyı ve sanatı aynı ekosistemde buluşturan deneysel sistemler, sahne akışları ve çok formatlı anlatılar tasarlıyorum.

---

### 📊 İstatistikler

| 15+ | 4+ | 10+ | ∞ |
|:---:|:---:|:---:|:---:|
| Yıllık Deneyim & Arayış | Temel Eksen: Bilinç · AI · Sanat · Sistem | Aktif Araştırma / Proje Alanı | Kavis, Soru ve Deneme |

---

### 🔬 Etki Alanları

**🧠 Bilinç & CCFy Araştırmaları** `Teorik / Deneysel`
> CCFy ile bilinç, karar ve deneyim "eğrilik" üzerinden modelleniyor. Simülasyon protokolleri ve açık bilim çıktıları.

**🤖 LLM & Çok Ajanlı Sistemler** `Uzman`
> FyCore ekosistemi: orkestrasyon, karar motorları, çok ajanlı iş akışları. Prompt mimarisi ve sistem tasarımı çekirdek becerilerimden.

**🎨 Görsel Anlatı & AI Art** `İleri`
> Realism Caricature, Neo-Paint Smudge stil kodları. Tarih ve sanatı AI anlatıyla birleştiren sAIvador pAIcasso hattı.

**🎙️ Podcast & Ses Evrenleri** `Deneyimsel`
> ElevenLabs tabanlı ses tasarımı, dramatik diyalog formatları. Dinlenebilir düşünce alanları.

**🧩 Sistem Tasarımı & Otomasyon** `İleri`
> Araştırma ve içerik süreçleri için modüler otomasyon zincirleri. FyCore bu hattın çekirdeği.

**📚 Anlatı Tasarımı & Eğitim Formatları** `Sürekli`
> Rafy Planet (çocuk evreni), sAIvador pAIcasso (YouTube), bilinç/AI ekseninde içerik serisi. TR/EN.

---

### 🏗️ Ekosistem — FyCore Cognitive OS

```
Sezgi → Analiz → Karar → Etik Denetim → Çıktı
  │        │        │           │           │
CCFy    Bayesian  Prefrontal  SymboLAB   CosmicLAB
```

| Lab | Rol |
|-----|-----|
| 🏭 **AidaLAB** | Bilişsel motor · Ordinaryus 6-agent · FyCore OS |
| ⚖️ **SymboLAB** | Sembolik karar · Etik anayasa · Veto |
| 🎨 **CosmicLAB** | Sunum katmanı · Sonra Art · Creative pipeline |
| ⚙️ **kernel** | RAG sistemi · DFP pipeline · Bellek mimarisi |

### Öne Çıkan Projeler

**🎨 SonRa ART** — Sanatı statü değil, bilinç alanı olarak yeniden kurgulayan atölye. → [sonra.art](https://www.sonra.art)

**🧠 CCFy** — Bilinç ve karar süreçlerini "eğrilik" kavramıyla ele alan IP çerçevesi. → [ccfy-framework](https://github.com/fthynn/ccfy-framework)

**🖼️ sAIvador pAIcasso** — Tarih + sanat tarihi + AI görsel anlatı. YouTube · Instagram · X.

**🪐 Rafy Planet** — Çocuklar için AI destekli içerik evreni.

---

### 🛠 Stack

`Python` · `FastAPI` · `React` · `Next.js` · `Docker` · `Qdrant` · `LangGraph` · `ElevenLabs` · `ComfyUI` · `Prolog`

<div align="center">

*"Bir sonraki kıvrımı beraber tasarlayalım."*
📬 [researcher@sonra.art](mailto:researcher@sonra.art)

</div>

</details>

---

<details>
<summary><strong>🇬🇧 English</strong></summary>

**Interdisciplinary Creative Technologist · Consciousness Researcher**

*Not here to consume time — here to reshape the curvature.* ☕

Founder of **SonRa ART** · Developer of **CCFy**. I design experimental systems, narrative pipelines, and multi-format productions that merge consciousness research, AI, and art into one ecosystem.

---

### 📊 Metrics

| 15+ | 4+ | 10+ | ∞ |
|:---:|:---:|:---:|:---:|
| Years of Experience & Exploration | Core Axes: Consciousness · AI · Art · Systems | Active Research / Project Areas | Curvatures, Questions & Experiments |

---

### 🔬 Spheres of Influence

**🧠 Consciousness & CCFy Research** `Theoretical / Experimental`
> CCFy models consciousness, decision-making and experience through "curvature." Simulation protocols and open science outputs.

**🤖 LLM & Multi-Agent Systems** `Expert`
> FyCore ecosystem: orchestration, decision engines, multi-agent workflows. Prompt architecture and system design are core competencies.

**🎨 Visual Narrative & AI Art** `Advanced`
> Realism Caricature, Neo-Paint Smudge style codes. The sAIvador pAIcasso pipeline merges history and art via AI-driven narrative.

**🎙️ Podcast & Sonic Universes** `Experimental`
> ElevenLabs-based sound design, dramatic dialogue formats. Listenable thinking spaces.

**🧩 System Design & Automation** `Advanced`
> Modular automation chains for research and content pipelines. FyCore is the core of this track.

**📚 Narrative Design & Educational Formats** `Ongoing`
> Rafy Planet (children's universe), sAIvador pAIcasso (YouTube), consciousness/AI content series. TR/EN bilingual.

---

### 🏗️ Ecosystem — FyCore Cognitive OS

```
Intuition → Analysis → Decision → Ethical Audit → Output
    │            │          │             │            │
  CCFy       Bayesian   Prefrontal    SymboLAB     CosmicLAB
```

| Lab | Role |
|-----|------|
| 🏭 **AidaLAB** | Cognitive engine · Ordinaryus 6-agent · FyCore OS |
| ⚖️ **SymboLAB** | Symbolic decision · Ethical constitution · Veto |
| 🎨 **CosmicLAB** | Presentation layer · Sonra Art · Creative pipeline |
| ⚙️ **kernel** | RAG system · DFP pipeline · Memory architecture |

### Featured Projects

**🎨 SonRa ART** — Creative studio reframing art as consciousness and status, not aesthetics. → [sonra.art](https://www.sonra.art)

**🧠 CCFy** — IP framework modeling consciousness and decisions through curvature theory. → [ccfy-framework](https://github.com/fthynn/ccfy-framework)

**🖼️ sAIvador pAIcasso** — History + art history + AI visual narrative. YouTube · Instagram · X.

**🪐 Rafy Planet** — AI-powered content universe for children.

---

### 🛠 Stack

`Python` · `FastAPI` · `React` · `Next.js` · `Docker` · `Qdrant` · `LangGraph` · `ElevenLabs` · `ComfyUI` · `Prolog`

<div align="center">

*"Let's architect the next curvature together."*
📬 [researcher@sonra.art](mailto:researcher@sonra.art)

</div>

</details>
